  ...
  return (
    <div className='rounded shadow-sm shadow-secondary-dark overflow-hidden  bg-primary dark:bg-primary-dark flex flex-col h-full'>
        <div className='aspect-video relative'>
            { !thumbnail ? (<div className='w-full h-full flex items-center justify-center text-secondary-dark opacity-50 font-semibold'>
                No Image
            </div>) : (<Image src={imageUrl as string} alt='Thumbnail' fill />)}
        </div>
        ...
        <Link href={'/' + slug}>
            ...
            <p className='text-secondary-dark-dark dark:text-secondary-light'> { trimText(meta, 70) } </p>  
        </Link>
        ...
        <div className='flex justify-end items-center h-8 mt-auto space-x-4  text-primary-dark dark:text-primary'>
            { busy ? (<span className='animate-pulse '> Removing </span>) : (<>
                <Link href={'/admin/post/update/' + slug} className='hover:underline'> Edit </Link>
